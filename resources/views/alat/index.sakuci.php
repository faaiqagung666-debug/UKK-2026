...
@section('content')
    ...
    <section class="text-center py-3 py-lg-4 hero-wrap">
        <span class="badge rounded-pill badge-brand px-3 py-2 mb-3">Manajemen Alat</span>

        <h1 class="display-6 fw-bold mb-2">
            Daftar <span class="text-brand">Alat</span>
        </h1>

        <p class="text-secondary mx-auto mb-3" style="max-width: 560px;">
            Kelola data alat/barang sekolah — tambah, ubah, atau hapus alat sesuai kebutuhan.
        </p>

        <a href="{{ route('alat.create') }}" class="btn btn-brand px-4">+ Tambah Alat</a>
    </section>
            ...
            <table class="table table-sm table-brand align-middle mb-0">
                ...
                <tbody>
                    ...
                    @foreach ($datal as $alats)
                        <tr>
                            <td>{{ $no++ }}</td>
                            <td class="fw-semibold">{{ $alats->nama_alat }}</td>
                            <td><code class="inline">{{ $alats->kode_alat }}</code></td>
                            <td>
                                @php
                                    $namaKategori = '-';
